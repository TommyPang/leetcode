...
class Solution {
    ...
    public int numTrees(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            // i-j-1 is the numebr of possible nodes in the right subtree
            // multiply them together(dp[j] * dp[i - j - 1]) gives the result for n==i
            // left subtree size == j and right subtree size == i-j-1;
            for (int j = 0; j < i; j++) {
                dp[i] += dp[j] * dp[i - j - 1];
            }
        }
